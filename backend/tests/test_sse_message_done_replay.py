"""message_done must be replayable from Redis."""
...
@pytest.mark.asyncio
async def test_reconnect_replays_message_done():
    ...
    request_id = f"done-{uuid.uuid4().hex}"
    try:
        await publish_sse_event(request_id, "message_done", {"message": "final text"})
        ...
        assert "final text" in resp.text
    finally:
        await close_redis()
