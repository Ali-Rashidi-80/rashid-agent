...
client = OpenAI(
    ...
    base_url="https://api.metisai.ir/api/v1/wrapper/grok",    #Grok Api
    
    #base_url="https://api.metisai.ir/openai/v1",    #OpenAi  &  gemini models

    api_key="***REDACTED_METIS_KEY***" # کلید خود را اینجا قرار دهید

)

#GEMINI_key = ""

#GEMINI_ENDPOINT = f"https://youapi.ir/generate?key={GEMINI_key}"  #اگه میخواین فیلتر شکن نزنید
...
def fix_and_parse_json(response_data):
    ...
    # بهبود هوشمند برای بستن رشته‌های ناقص
    ...
    try:
        ...
        end_index = min(len(json_text), error_index + 50)
        ...
        # بازگرداندن دیکشنری خطا برای جلوگیری از کرش سیستم
...
def payload (user_text: object)  -> object:
    ...
    dastor = f"""
       ...
       type: 
        ...
        اگه باگی داره بگی
        -----------------------
        {json.dumps(json_formatMain, ensure_ascii=False, indent=4)}
        -----------------------
        message:
       ...
       "هرگاه کدهای پایتون را در قالب JSON برای من می‌فرستی، باید تمامی کاراکترهای خاص (مثل \n، \" و غیره) را در بخش new_code به صورت کامل Escape کنی. اگر در کدی از Regex استفاده شده (مثل (.*?))، آن را به صورت (.*?) در JSON بنویس تا ساختار JSON شکسته نشود. خروجی باید کاملاً Valid JSON باشد."
